...
def threshold_adaptive(img, k=3, C=5, method='gaussian', maxVal=255):
    ...
    if (is_grayscale(img)):
        return cv2.adaptiveThreshold(img, maxVal, methods[method], cv2.THRESH_BINARY, k, C)
    else:
        return color(cv2.adaptiveThreshold(grayscale(img), maxVal, methods[method], cv2.THRESH_BINARY, k, C))
...
def draw_linesP(img, linesP, params=['angle', 'length', 'orient'], color=(0,255,0), thickness=3):
    if linesP == None:
        return img
    units = {
        "angle": "d",
    ...
    for l in linesP:
        ...
        cv2.putText(img, ":".join(all_text), l['mid'], cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255))
    return img
...
def draw_data(img, data_by_entry, draw_boxes=True, draw_text=False, color_boxes=None, color_text=None, offset_text=False):
    if data_by_entry == []:
        return img
    ...
    for entry in data_by_entry:
        ...
        if draw_text:
            if (offset_text):
                img = cv2.putText(img, entry['text'], bbox_start, cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=color_text, thickness=2)
            else:
                img = cv2.putText(img, entry['text'], (bbox_start[0], bbox_end[1]), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, color=color_text, thickness=2)
    return img